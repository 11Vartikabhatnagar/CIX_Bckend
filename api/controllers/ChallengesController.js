...
module.exports = {
  create: async (req, res) => {
    ...
    try {
      ...
      // trigger email & push notification for users
  ...
  discoverChallenge: async (req, res) => {
    try {
      ...
      if (req.body.filter.uns_proposal) {
        conditionClause = conditionClause.concat(
          " ",
          `AND Challenges.uns_proposal = "true"`
        );
      }
      else{
        conditionClause = conditionClause.concat(
          " ",
          `AND Challenges.uns_proposal = "false"`
        );
      }
...
async function triggerNotificationForNewChallenge(challengeData, req) {
  ...
  providerIdBasedOnSector.forEach(async (e) => {
    ...
    // push notification
    await sails.helpers.notificationHandler("ON_NEW_CHALLENGE", {
      message_payload: {
        type: "obj",
        payloads: {
      ...
      userId: providerData.frg_user_id.id,
    });
  });
