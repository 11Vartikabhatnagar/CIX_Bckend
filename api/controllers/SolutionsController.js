...
module.exports = {
  ...
  getdata :async (solutionData,res)=>{
    let currentChallenge = await Challenges.findOne({ id: '195' })
        .populate("frg_challenge_sector_id")
        .populate("anchors")
        .populate("solutions")
        .populate("smart_city");
        res.send({data:currentChallenge})
  }
...
async function triggerNotificationForNewSolutionUnderSector(solutionData, req) {
  if (solutionData && solutionData.status === "active") {
    ...
    seekerIdBasedOnSector.forEach(async (e) => {
      ...
      // push notification
      await sails.helpers.notificationHandler(
        "ON_SOLUTION_ADDED_UNDER_SECTOR",
        {
          message_payload: {
            type: "obj",
            payloads: {
              provider_slug: solutionData.frg_provider_id.slug,
              company_name: companyName,
          ...
          userId: seekerData.frg_user_id.id,
        }
      );
    });
  }
...
async function triggerNotificationForNewSolutionUnderChallenge(solutionData,req) {
    ...
    solutionData.challenges.forEach(async (challenge) => {
      ...
      currentChallenge.anchors.forEach(async (anchor) => {
        ...
        if (anchor.status === "active") {
          ...
          // push notification
          await sails.helpers.notificationHandler(
            "ON_SOLUTION_ADDED_UNDER_CHALLENGE",
            {
              message_payload: {
                type: "obj",
                payloads: {
