...
module.exports = {
  create: async (req, res) => {
    const proposal_data = await Proposal.create(req.body.data).fetch();
    try {
      res.send({
        status: true,
      ...
      triggerNotificationForRcvdProposal(proposal_data);
    } catch (err) {
  ...
  pilotIIcUpload: async (req, res) => {
    try {
      const temp = await Proposal.findOne({ id: req.body.slug });
      const pilotDocs = req.file("pilot_iic");
      if (!temp) {
        return res.send({ status: false, msg: "Proposal not found", data: [] });
      } else if (!pilotDocs._files && !req.body.pilotDocs) {
        sails.log.warn("No files uploaded");
        ...
        res.send({
          ...
          msg: "Docs updated successfully",
          data: await Proposal.update({ id: temp.id })
            .set({ pilot_iic: docUploads.urlPaths })
            .fetch(),
        });
      }
    } catch (err) {
  ...
  addToPilotIICUpload: async (req, res) => {
    try {
      const temp = await Proposal.findOne({ id: req.body.slug });
      const additionalDocs = req.file("pilot_iic");
      if (!temp) {
        return res.send({ status: false, msg: "proposal not found", data: [] });
      } else if (!additionalDocs._files && !req.body.additionalDocs) {
        sails.log.warn("No files uploaded");
        ...
        res.send({
          status: true,
          msg: "Pilot iic updated successfully",
          data: await Proposal.update({ id: temp.id })
            .set({
              pilot_iic: [...temp.pilot_iic, ...docUploads.urlPaths],
            })
            .fetch(),
        });
      }
  ...
  removeFromPilotIICUpload: async (req, res) => {
    try {
      const fileRemovePaths = req.body.data.remove_paths || null;
      const currentUploads = await Proposal.findOne({
        select: ["pilot_iic"],
      ...
      let modifiedUploads = currentUploads.pilot_iic;
      if (
        fileRemovePaths &&
        fileRemovePaths.length > 0 &&
        currentUploads.pilot_iic.length > 0
      ) {
        fileRemovePaths.forEach((removePath) => {
          let removePathFull = path.join(
            rootDir.toString(),
  ...
  rejectProposalNotification: async (req, res) =>{
    var post_data = req.body;
    const status = await Proposal.updateOne({id:req.body.proposal_id}).set({
      proposal_stage:'Reject'
    });
    res.send({
      status:true,
      data:status
    })
    await sails.helpers.notificationHandler("ON_PROPOSAL_REJECTION", {
      message_payload: {
        type: "obj",
        payloads: post_data
      },
      userId: post_data.innovator_id,
    });
  }
};

// push notifcation for Proposal creation
async function triggerNotificationForRcvdProposal(prop_det){
  const prop_details = await Proposal.findOne({ id: prop_det.id }).populate("frg_provider_id").populate("frg_anchor_id");
  const challenge_detail = await Anchors.findOne({ id: prop_details.frg_anchor_id.id }).populate("frg_challenge_id").populate("frg_seeker_id");
  var post_data ={};
  post_data.anchor_id = prop_details.frg_anchor_id.id ;
  post_data.user_slug = challenge_detail.frg_seeker_id.slug;

  if(prop_details.frg_provider_id.role == 'startup'){
    post_data.provider_name = prop_details.frg_provider_id.startup_company_name;
    post_data.type ="startup"
  }
  else if(prop_details.frg_provider_id.role == 'enterprise'){
    post_data.provider_name = prop_details.frg_provider_id.enterprise_company_name;
    post_data.type ="enterprise"
  }
  else{
    post_data.provider_name = prop_details.frg_provider_id.fullname;
    post_data.type ="induvidual"
  }
  
  post_data.challenge_name = challenge_detail.frg_challenge_id.name;

  console.log(post_data)

  await sails.helpers.notificationHandler("ON_NEW_PROPOSAL", {
    
    message_payload: {
      type: "obj",
      payloads: post_data
    },
    userId: challenge_detail.frg_seeker_id.frg_user_id,
  });
}
